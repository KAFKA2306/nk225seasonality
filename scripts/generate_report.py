...
from src import SystemConfig, AnalysisPipeline

async def generate_report():
    print("Starting Report Generation...")
    config = SystemConfig()
    
    # --- 1. Existing Valuation Analysis (Refactored to use System Config) ---
    print("Fetching Valuation Data...")
    
    # Use config values
    years = config.valuation.years_for_analysis
    eps = config.valuation.assumed_eps
    ...
    for date, row in df.iloc[::-1].iterrows():
        ...
        status = (
            "Significantly Overvalued" if div > 20
            else "Overvalued" if div > 10
            else "Significantly Undervalued" if div < -20
            else "Undervalued" if div < -10
            else "Fairly Valued"
        )
    ...
    # Override output_dir to write images directly to docs/
    config.output_dir = Path("docs")
    config.output_dir.mkdir(exist_ok=True)
    
    pipeline = AnalysisPipeline(config)
    end_date = datetime.now()
    start_date = end_date - timedelta(days=10*365) # Analyze 10 years
    
    results = await pipeline.run_full_analysis(
        start_date, end_date, save_results=True, skip_storage=True
    )
    ...
    <div class="section">
        ...
        <div class="table-container">
            <table>
                ...
                {"".join(valuation_rows[:12])} <!-- Show last 12 months -->
            </table>
        </div>
    </div>
